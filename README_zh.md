...
> **命名说明：** VarSim 是一个 HGVS 变异*工具集*。它与读长模拟器"VarSim"（PMID: 25524895）无关，请勿混淆。

VarSim 是一个全面的 HGVS 变异命名工具集——提供解析、验证、标准化、回译、格式转换、提取、Liftover、转录和翻译功能。全部依托 NCBI Entrez 实现。
...
| `normalizer.normalize_3prime_shift(hgvs, ref_seq)` | 将变异尽量向 3′ 端移动 |
| `normalizer.ins_to_dup(hgvs, ref_seq)` | 在适当情况下将插入转换为重复 |
...
| `transcription.get_cds_exon_map(gene)` | 外显子结构映射（cDNA + 基因组坐标） |
...
| `translation.translate_variants(c_hgvs_list, gene)` | 批量翻译多个 c.HGVS 字符串 |
...
| **标准化** | `normalize(hgvs, ref_seq?)` / `normalizer.normalize_3prime_shift(...)` / `normalizer.ins_to_dup(...)` | 完整标准化 / 3′位移 / ins→dup |
...
| **转录** | `c_to_g(c_hgvs, gene)` / `g_to_c(g_hgvs, gene)` / `transcription.get_cds_exon_map(gene)` | 编码↔基因组 / 外显子结构 |
